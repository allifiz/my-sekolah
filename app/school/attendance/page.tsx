...
export default async function AttendancePage({
  ...
  searchParams: Promise<{ classGroupId?: string; date?: string; error?: string; success?: string }>;
}) {
  ...
  if (selectedClassGroup) {
    ...
    const [enrollments, foundSession] = await Promise.all([
      ...
      prisma.attendanceSession.findUnique({
        where: { classGroupId_date: { classGroupId: selectedClassGroup.id, date } },
        include: { records: true, submittedBy: { include: { user: true } } },
      }),
    ]);
  ...
  return (
    <div className="admin-page">
      <header className="page-header">
        <div>
          <span className="eyebrow">Operasional harian</span>
          <h1>Absensi Rombel</h1>
          <p>Catat kehadiran siswa per tanggal. Wali kelas hanya melihat rombel yang ditugaskan kepadanya.</p>
        </div>
      </header>
      ...
      {params.success ? <section className="panel section-panel"><strong>Berhasil:</strong> Absensi sudah tersimpan.</section> : null}

      <section className="panel section-panel">
        <h2>Pilih Rombel dan Tanggal</h2>
        {classGroups.length === 0 ? <p>Belum ada rombel yang dapat kamu akses.</p> : (
          <form method="get" className="admin-form">
            <label>Rombongan belajar
              <select name="classGroupId" defaultValue={selectedClassGroup?.id}>
      ...
      {selectedClassGroup ? (
        <section className="panel section-panel">
          <h2>{selectedClassGroup.gradeLevel.name} · {selectedClassGroup.name}</h2>
          <p>{selectedClassGroup.academicYear.name} · {selectedDate}</p>
          {existingSession ? <p>Terakhir disimpan oleh {existingSession.submittedBy.user.name ?? existingSession.submittedBy.user.email}.</p> : <p>Belum ada absensi tersimpan untuk tanggal ini.</p>}
          {attendanceRows.length === 0 ? <p>Belum ada siswa aktif dalam rombel ini.</p> : (
            <AttendanceForm
              classGroupId={selectedClassGroup.id}
              date={selectedDate}
              initialRows={attendanceRows}
              isCorrection={Boolean(existingSession)}
            />
          )}
        </section>
      ) : null}
