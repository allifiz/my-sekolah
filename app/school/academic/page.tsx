    ...
    <div className="admin-page">
      <header className="page-header">
        <div>
          ...
          <p>Atur periode akademik aktif yang akan menjadi dasar kelas, jadwal, absensi, dan penilaian.</p>
        </div>
      </header>
      ...
      <section className="panel section-panel">
        ...
        <form action={createAcademicYear} className="admin-form">
          ...
          <button type="submit" className="primary-button">Simpan Tahun Ajaran</button>
        </form>
      </section>

      <section className="panel section-panel">
        <h2>Tambah Semester</h2>
        {years.length === 0 ? <p>Buat tahun ajaran terlebih dahulu.</p> : (
          <form action={createSemester} className="admin-form">
            <label>Tahun ajaran
            ...
            <label>Semester
              <select name="type" required defaultValue="ODD">
            ...
            <button type="submit" className="primary-button">Simpan Semester</button>
          </form>
        )}
      </section>

      <section className="panel section-panel">
        <h2>Daftar Periode Akademik</h2>
        {years.length === 0 ? <p>Belum ada tahun ajaran.</p> : years.map((year) => (
          <article key={year.id} className="panel section-panel">
            <div className="page-header">
              <div>
                <h3>{year.name} {year.isActive ? "· Aktif" : ""}</h3>
              ...
              {!year.isActive ? (
                <form action={activateAcademicYear}>
                  ...
                  <button type="submit" className="secondary-button">Aktifkan</button>
                </form>
              ) : null}
            </div>

            {year.semesters.length === 0 ? <p>Belum ada semester.</p> : (
              <div className="stats-grid">
                {year.semesters.map((semester) => (
                  <article key={semester.id}>
                    <span>{semester.name}</span>
                    <strong>{semester.isActive ? "Aktif" : semester.type}</strong>
                    ...
                    {!semester.isActive ? (
                      <form action={activateSemester}>
                        ...
                        <button type="submit" className="secondary-button">Aktifkan Semester</button>
                      </form>
                    ) : null}
                  </article>
                ))}
